export function addMetadataToPng(pngArrayBuffer, metadataObj) {
    ...
    const chunkData = concatUint8Arrays([keyword, nullByte, textData]);

    const chunkType = encoder.encode('tEXt');
    const crcData = concatUint8Arrays([chunkType, chunkData]);
    ...
    const chunk = concatUint8Arrays([chunkLength, chunkType, chunkData, crcBytes]);

    const pngBytes = new Uint8Array(pngArrayBuffer);

    const ihdrEnd = 8 + 4 + 4 + 13 + 4;

    const newPng = concatUint8Arrays([
...
export function extractMetadataFromPng(file) {
